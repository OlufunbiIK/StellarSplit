...
describe('SplitTemplateController', () => {
  ...
  beforeEach(async () => {
    ...
    controller = module.get<SplitTemplateController>(SplitTemplateController);
  ...
  describe('create', () => {
    it('should create a new split template', async () => {
      ...
      expect(service.create).toHaveBeenCalledWith(mockUser.wallet, createDto);
  ...
  describe('findAll', () => {
    it('should return all templates for a user', async () => {
      ...
      mockSplitTemplateService.findAllForUser.mockResolvedValue(templates);

      const result = await controller.findAll({ user: mockUser } as any);

      expect(service.findAllForUser).toHaveBeenCalledWith(mockUser.wallet);
  ...
  describe('createSplit', () => {
    it('should create a split from template', async () => {
      ...
      expect(service.createSplitFromTemplate).toHaveBeenCalledWith(templateId, undefined);
    ...
    it('should create a split from template with overrides', async () => {
      ...
      const result = await controller.createSplit(templateId, overrideDto);

      expect(service.createSplitFromTemplate).toHaveBeenCalledWith(templateId, overrideDto);
      expect(result).toEqual(expectedSplit);
    });
  });
});
